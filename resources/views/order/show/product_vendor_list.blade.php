...
<form data-reset="false" data-page-reload="true" action="{{route('orders.vendor.assign_product_vendor_list', request()->route()->parameters())}}" method="post">
@csrf
        ...
        <tr class="cartTr">
            ...
            <td class="align-middle" style="text-align: left">
                ...
                <div style="margin-top: 5px; font-size: 12px; font-weight: bold;">
                    ...
                    <span class="badge-info" style="border-radius: 5px;min-width: 40px; padding: 5px; margin-left: 5px; margin-right: 5px;">৳
            ...
            <td class="align-middle" style="text-align: center; width: 150px;">
                ...
                <select name="product_vendor[{{$item->uuid}}]" class="custom-select {{$item->product->vendor_id ? '' : 'is-invalid'}}" style="padding: 2px;" name="" id="">
                    <option value="">Select Vendor</option>
                    @foreach ($vendors as $vendor)
                        <option value="{{$vendor->id}}" {{$vendor->id == $item->product->vendor_id ? 'selected' : ''}}>{{$vendor->name}}</option>
                    @endforeach
                </select>
            </td>
        </tr>
